  ...
  template: `
    <mat-form-field fxFlex="100" [formGroup]="fg_fields">
        ...
        <button
          ...
          matSuffix
          *ngIf="this.fg_fields.controls['Ftl'].value && !this.fg_fields.controls['Ftl'].errors"
          (click)="openFtl()">
          <span class='fa_regular'><fa-icon [icon]="['fas', 'money-check-alt']"></fa-icon></span>
        </button>
...
export class JobsFormFtlComponent {
  ...
  openFtl() {
    ...
    if (ftl) {
      window.open(
        `https://ftl.ebusiness.slb.com/inbox.cfm?srch=1&type=t&ticket=${ftl}`,
        '_blank'
      );
    }
  }

...
// form services
import { HarcsFormInitService } from './form-services/harcs-form-init.service';
...
@Component({
  selector: 'app-harcs-form',
  styleUrls: ['harcs-form.component.scss'],
  templateUrl: './harcs-form.component.html',
  providers: [HarcsFormHttpService, HarcsFormInitService]
})
export class HarcsFormComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private formInitService: HarcsFormInitService,
    public formRef: MatDialogRef<HarcsFormComponent>,
    // private formSizeService: PeopleFormSizeService,
    @Inject(MAT_DIALOG_DATA) public data: { mode: FormMode; item?: HarcItem }
  ) {}
  ...
  createFormGroups(m: FormMode, it: HarcItem, lo: number) {
    // create 1 form group
  ...
  updateDataItem(updatedFields: HarcItem) {
  ...
  onSelectUser(selected: number[]) {
    ...
    this.fg_fields.get('PICId').patchValue(selected[0]);
  }
